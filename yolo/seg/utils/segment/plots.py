...
def delete_object(img, masks, colors, index, alpha=0.6):
    """
    Args:
    ...
    colors = colors[0, None, None, 2]
    # [n, h, w, 1]
    masks = masks[index:index+1, :, :, None]
    masks_color = masks.repeat(1, 1, 1, 3) * colors * alpha
    inv_alph_masks = masks * (-alpha) + 1
    masks_color_summand = masks_color[0]
    if num_masks > 1:
        inv_alph_cumul = inv_alph_masks.cumprod(dim=0)
...
def plot_masks(img, masks, colors, index, alpha=1):
    """
    Args:
        img (tensor): img is in cuda, shape: [3, h, w], range: [0, 1]
        masks (tensor): predicted masks on cuda, shape: [n, h, w]
    ...
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    img_gpu = torch.zeros(img.shape).to(device)
    num_masks = len(masks)
    if num_masks == 0:
        return img.permute(1, 2, 0).contiguous().cpu().numpy() * 255
